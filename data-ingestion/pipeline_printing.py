"""
Unison Orchestration — Additive Manufacturing CSV Ingestion Pipeline
=====================================================================
Reads structured CSV files containing G-code tokens, slicing parameters,
thermal profiles, and polymer processing specifications.
Groups rows into dense text chunks and embeds into unison_additive_manufacturing.

Usage:
  python3 pipeline_printing.py --csv raw_sources/additive_thermal_profiles.csv
  python3 pipeline_printing.py --csv raw_sources/additive_thermal_profiles.csv --rows-per-chunk 5
...
log = logging.getLogger("unison.printing")

COLLECTION_NAME  = "unison_additive_manufacturing"
X402_PRICE       = 0.050
TIER             = "premium"
DOMAIN           = "3d_printing"
...
def _row_to_text(row: dict) -> str:
    """Convert a CSV row to a dense semantic text block for embedding."""
    parts = []
...
def csv_to_chunks(
    ...
    for i in range(0, len(reader), rows_per_chunk):
        ...
        first_id = batch[0].get("profile_id") or batch[0].get("primitive_id") or f"ROW-{i}"
        asset_id = f"3D-MANUFACTURING-{first_id}"

        # Extract key fields for SKU metadata
        polymers  = list({r.get("polymer", "") for r in batch if r.get("polymer", "")})
        processes = list({r.get("process", "")  for r in batch if r.get("process", "")})
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Unison Additive Manufacturing CSV ingestion")
    parser.add_argument("--csv", required=True, help="Path to additive manufacturing CSV")
    parser.add_argument("--rows-per-chunk", type=int, default=5, dest="rows_per_chunk",
                        help="CSV rows per text chunk (default: 5)")
    parser.add_argument("--provenance", default="industrial_slicer_bounds",
                        help="Data provenance label stored in SKU payload")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    log.info("=== Unison Additive Manufacturing Ingestion Pipeline START ===")
    ...
    upsert_manufacturing(embedded, skus, qdrant_client)
