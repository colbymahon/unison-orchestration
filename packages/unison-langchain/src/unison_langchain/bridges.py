"""Framework bridges — LangChain and LlamaIndex one-line integration surfaces."""
...
def _forward_headers(
    ...
    *,
    ...
    headers = {
        ...
        "User-Agent": f"UnisonLangChain/{agent_id.strip()}",
    }
    if session_id:
...
def fetch_tsv_stream(
    *,
    ...
    """Single-hop edge retrieval — forwards X-Agent-ID to the Unison Anycast proxy."""
...
class UnisonLangChainBridge:
    """
    LangChain-native bridge — TSV streaming as retriever-compatible documents.

    Example:
        bridge = UnisonLangChainBridge(agent_id="my-langchain-agent")
        docs = bridge.as_retriever_invoke("Tesla resonant coil parameters")
    ...
    def as_retriever_invoke(self, query: str) -> list[dict[str, Any]]:
        """Document-shaped dicts for LangChain Runnable chains."""
    ...
    def as_langchain_retriever(self) -> Any:
        """Return UnisonX402Retriever with full x402 + churn telemetry."""
...
class UnisonLlamaIndexBridge:
    ...
    def as_query_engine(self) -> Any:
        """Return a minimal LlamaIndex CustomQueryEngine when llama-index-core is installed."""
        try:
